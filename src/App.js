...
class App extends React.Component {
    ...
    createUser = async(id) => {
        let result = await axios.get(`https://109.195.67.182:3002/createUser?user_id=${id}`);
        console.log(result.data.user)
        this.setState({balance: result.data.user.balance})
    }

    updateUserBalance = async(id) => {
        let result = await axios.get(`https://109.195.67.182:3002/updateUserBalance?user_id=${id}`);
